...
class SummaryLengthTest extends NodeTestBase {
  ...
  public function testSummaryLength() {
    ...
    $this->assertTrue(strlen($content['body'][0]['#markup']) < 600, 'Teaser is event than 600 characters long.');
    ...
    $content = $this->drupalBuildEntityView($node, 'teaser');
    $this->assertTrue(strlen($content['body'][0]['#markup']) < 200, 'Teaser is event than 200 characters long.');
    $this->setRawContent($renderer->renderRoot($content));
